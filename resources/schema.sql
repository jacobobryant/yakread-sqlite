...
CREATE TABLE user (
  ...
  email TEXT NOT NULL,
  roles BLOB, -- [:set [:enum :admin]]
  joined_at INT,
  digest_days BLOB, -- [:set ::day]
  send_digest_at TEXT,
  timezone TEXT,
...
CREATE TABLE sub (
  ...
  pinned_at INT,
  record_type INT NOT NULL CHECK (record_type IN (0, 1, 2)),
  feed_id BLOB,
  email_from TEXT,
...
CREATE TABLE item (
  ...
  image_url TEXT,
  paywalled INT,
  record_type INT NOT NULL CHECK (record_type IN (0, 1, 2, 3)), -- :feed (0), :email (1), :direct (2)
  feed_id BLOB,
  feed_guid TEXT,
  email_sub_id BLOB,
  ...
  direct_candidate_status INT CHECK (direct_candidate_status IN (0, 1, 2)),
  FOREIGN KEY(feed_id) REFERENCES feed(id),
...
CREATE TABLE bulk_send (
  ...
  digests BLOB NOT NULL -- [:vector :uuid]
) STRICT;
...
CREATE TABLE reclist (
  ...
  user_id BLOB NOT NULL,
  created_at INT NOT NULL,
  clicked BLOB NOT NULL, -- [:set :uuid]
  FOREIGN KEY(user_id) REFERENCES user(id)
) STRICT;
...
CREATE TABLE ad (
  ...
  session_id TEXT,
  payment_method TEXT,
  card_details BLOB, -- [:map [:brand :string] [:last4 :string] [:exp_year :int] [:exp_month :int]]
  FOREIGN KEY(user_id) REFERENCES user(id)
) STRICT;

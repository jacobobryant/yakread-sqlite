...
(defn seed! [{:keys [biff/node] :as ctx}]
  ...
  ;; Write to XTDB (for resolvers that still query XTDB)
  (biffx/submit-tx ctx (seed-tx))
  ;; Write to SQLite (for resolvers that query SQLite)
  (seed-sqlite! ctx)
  ;; Wait for XTDB transaction to be indexed
  (Thread/sleep 2000)
